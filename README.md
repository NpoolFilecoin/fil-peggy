...
## ${\color{blue}{9 \ Try \ our \ MVP \ version \ dApp}}$
...
* ```yarn build```: build dApp which can be loaded through chrome extension
* Enter ```chrome://extensions/``` in chrome address bar, enable ```developer mode```
* ```Load uncompressed extension``` to try, folder to be loaded should be ```dist``` directory generated by ```yarn build```

Note: ```Currently we just store private key in localStorage, in production version this will be protected by a personal encryption key, and stored with chrome.storage```
